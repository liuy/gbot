...
func (s *Store) AddFact(content string) (factID int64, inserted bool, err error) {
	...
	// All 4 SQL ops must be atomic: if the second INSERT fails after the first,
	// we'd have a fact row with no FTS index entry. Wrap in one transaction.
	tx, err := s.db.Begin()
	if err != nil {
	...
	if rows == 0 {
		...
		if err := tx.Commit(); err != nil {
			return 0, false, fmt.Errorf("facts: commit (duplicate): %w", err)
		}
		return id, false, nil
	}
