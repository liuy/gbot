...
// TestSetContextUsed verifies context display update.
func TestSetContextUsed(t *testing.T) {
	t.Parallel()

	t.Run("shows used tokens", func(t *testing.T) {
		app := newTestApp(&tuiMockProvider{})
		app.SetContextUsed(100)
	...
	t.Run("context window from engine", func(t *testing.T) {
		app := newTestApp(&tuiMockProvider{})
		app.engine.UpdateAutoCompactConfig(engine.AutoCompactConfig{ContextWindow: 512000})
		app.SetContextUsed(100)
		if got := app.status.contextTotal; got != 512000 {
			t.Errorf("contextTotal = %d, want %d (from engine.ContextWindow)", got, 512000)
		}
	})
}
...
func TestReplState_PendingToolDone_UnknownID(t *testing.T) {
	...
	s.PendingToolDone("nonexistent", "output", false, 0, tool.SearchReadKind{})
...
func TestApp_FinishStream_WithTools(t *testing.T) {
	...
	app.spinner.Start()
	app.repl.PendingToolStarted("t1", "Read", "", `{}`, tool.SearchReadKind{})
	app.repl.PendingToolDone("t1", "contents", false, 0, tool.SearchReadKind{})
...
func TestApp_UpdateRepl_SubAgentToolFullLifecycle(t *testing.T) {
	...
	app.repl.PendingToolDone("call_agent1", "agent result", false, 500*time.Millisecond, tool.SearchReadKind{})
...
func TestApp_UpdateRepl_SubAgentTextIndentation(t *testing.T) {
	...
	// End the agent
	app.repl.PendingToolDone("call_agent1", "", false, 500*time.Millisecond, tool.SearchReadKind{})
